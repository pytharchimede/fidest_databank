...
class Helper
{
    ...
    // Méthode pour calculer le taux de rentabilité
    public static function calculerTauxRentabilite($totalDepenses, $totalFacture)
    {
        // Vérification pour éviter une division par zéro
        if ($totalFacture == 0) {
            return "Erreur : le total facturé ne peut pas être égal à zéro.";
        }

        // Calcul du taux de rentabilité
        $tauxRentabilite = (($totalFacture - $totalDepenses) / $totalFacture) * 100;

        // Retourner le taux de rentabilité avec 2 décimales
        return number_format($tauxRentabilite, 2) . '%';
    }
}
